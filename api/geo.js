// Country dialing code for the current visitor — Vercel Serverless Function
// GET /api/geo → { countryCode: "PL", callingCode: "+48" }
//
// Vercel sets the x-vercel-ip-country header from the visitor's IP in
// production (no external API needed). The header is mapped to the country's
// international calling code so the checkout page can pre-fill the phone
// field with e.g. +48 (Poland), +7 (Kazakhstan / Russia), +1 (USA), etc.
// Falls back to an ipapi.co lookup when the header is missing (local dev).
...
module.exports = async (req, res) => {
  ...
  let countryCode = String(req.headers['x-vercel-ip-country'] || '').toUpperCase();

  // Header is missing in local dev / previews — resolve the caller IP instead.
  if (!countryCode) {
    const ip = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim() ||
      (req.socket && req.socket.remoteAddress) || '';
    countryCode = await geoLookupByIp(ip);
  }

  return res.status(200).json({
    countryCode: countryCode,
    callingCode: COUNTRY_CALLING[countryCode] || ''
  });
};
